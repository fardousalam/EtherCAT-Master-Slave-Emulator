...
#define CAT_TYPE_TXPDO	  0x0033 // written by master
#define CAT_TYPE_RXPDO	  0x0032 // read by master
...
#define RX_PDO1_NAME_IDX        6
#define RX_PDO2_NAME_IDX        7
#define TX_PDO1_NAME_IDX	8
#define TX_PDO2_NAME_IDX	9
...
#define RXPDO_CAT_STRING  "ARDUINO OUTPUT" 
#define TXPDO_CAT_STRING  "ARDUINO INPUT"
// arduino transmit
#define TX_PDO1_NAME "OUT PIN"
#define TX_PDO2_NAME "STATUS"
// arduino recieves
#define RX_PDO1_NAME "IN PIN"
#define RX_PDO2_NAME "COMMAND"
...
void init_sii(ecat_slave *esc)
{
	...
	categories.rxpdo.entries = NR_PDOS;
	...
	for (pdoe_idx = 0;  pdoe_idx < NR_PDOS -1 ; pdoe_idx++) {
		init_pdo(&categories.rxpdo.pdo[pdoe_idx], 
			categories.rxpdo.pdo_index , pdoe_idx, 
			RX_PDO1_NAME_IDX + 1, 
			0, 8, 0);
	}
	init_pdo(&categories.rxpdo.pdo[CMD_PDO_IDX], 
			categories.rxpdo.pdo_index , STAT_PDO_IDX , 
			RX_PDO2_NAME_IDX + 1, 
			0, 32, 0);

	categories.txpdo_hdr.type = CAT_TYPE_TXPDO;
	categories.txpdo_hdr.size = sizeof(categories.txpdo)/2;
	categories.txpdo.entries = NR_PDOS;
	...
	for (pdoe_idx = 0;  pdoe_idx < NR_PDOS -1 ; pdoe_idx++) {
		init_pdo(&categories.txpdo.pdo[pdoe_idx], 
			categories.txpdo.pdo_index , pdoe_idx, 
			TX_PDO1_NAME_IDX + 1, 
			0, 8, 0);
	}
	init_pdo(&categories.txpdo.pdo[STAT_PDO_IDX], 
			categories.txpdo.pdo_index , CMD_PDO_IDX , 
			TX_PDO2_NAME_IDX + 1, 
			0, 32, 0);
}
